...
BEGIN
  ...
  FROM public.agents WHERE agent_type = 'spirit' AND status = 'active';
  SELECT count(*) INTO v_pending_agent_approvals
  FROM public.agent_action_logs WHERE status = 'pending_approval';
  SELECT count(*) INTO v_blocked_users
  FROM public.profiles WHERE approval_status IN ('rejected', 'blocked');
...
RETURNS TABLE (
  ...
  approval_status text, role text, access_tier text,
  created_at timestamp with time zone, microdao_count bigint
)
...
BEGIN
  PERFORM public.check_is_platform_admin();
  RETURN QUERY
  SELECT p.id, p.user_id, p.email, p.display_name, p.approval_status, p.role, p.access_tier, p.created_at,
...
RETURNS TABLE (
  id uuid, name text, slug text, type text, owner_id uuid,
  owner_email text, owner_name text, created_at timestamp with time zone,
  member_count bigint, agent_status text, has_spirit_agent boolean
)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, auth
AS $$
BEGIN
  PERFORM public.check_is_platform_admin();
  RETURN QUERY
  SELECT c.id, c.name, c.slug, c.type, c.owner_id, p.email, p.display_name, c.created_at,
    (SELECT count(*) FROM public.community_members cm WHERE cm.community_id = c.id),
    COALESCE((SELECT a.status::text FROM public.agents a WHERE a.community_id = c.id AND a.agent_type = 'spirit' LIMIT 1), 'none'),
    EXISTS (SELECT 1 FROM public.agents a WHERE a.community_id = c.id AND a.agent_type = 'spirit')
  FROM public.communities c
...
RETURNS TABLE (
  id uuid, user_id uuid, email text, display_name text, use_case text,
  requested_tier text, status text, created_at timestamp with time zone,
...
BEGIN
  PERFORM public.check_is_platform_admin();
  RETURN QUERY
  SELECT ar.id, ar.user_id, ar.email, ar.display_name, ar.use_case, ar.requested_tier,
    ar.status, ar.created_at, ar.reviewed_at, ar.reviewed_by
  FROM public.access_requests ar ORDER BY ar.created_at DESC;
END;
$$;
...
AS $$
DECLARE
  v_total_agents int; v_spirit_agents int;
  v_agents_by_status json; v_agents_by_type json;
  v_recent_logs json; v_recent_errors json; v_result json;
BEGIN
  PERFORM public.check_is_platform_admin();
  ...
  FROM (SELECT status, count(*) as count FROM public.agents GROUP BY status) t;
  SELECT json_object_agg(agent_type, count) INTO v_agents_by_type
  FROM (SELECT agent_type, count(*) as count FROM public.agents GROUP BY agent_type) t;
  SELECT COALESCE(json_agg(t), '[]'::json) INTO v_recent_logs
  FROM (
    SELECT l.id, l.agent_id, a.name as agent_name, l.action_type, l.status, l.created_at, c.name as community_name
  ...
  FROM (
    SELECT l.id, l.agent_id, a.name as agent_name, l.action_type, l.status, l.created_at,
